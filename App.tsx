...
import GrasslandScene from './components/VoxelWorld/GrasslandScene';
...
const App: React.FC = () => {
  ...
  const [sceneMode, setSceneMode] = useState<'autumn' | 'grassland'>('autumn');

  const handleListenToNature = async () => {
    setIsLoading(true);
    const description = sceneMode === 'autumn'
      ? "An autumn mountain voxel world with vibrant red, orange, and yellow leaves flying in the wind, a flowing blue river, wandering wildlife, under a golden sunset."
      : "A peaceful green grassland with rolling hills, blue sky, white clouds, gentle breeze, and cows grazing in the field.";
    ...
    <div className="relative w-full h-screen bg-gray-900 text-white font-sans overflow-hidden">
      ...
      <div className="absolute inset-0 z-0">
        <Canvas shadows camera={{ position: [30, 30, 30], fov: 35, near: 1, far: 200 }}>
          ...
          {sceneMode === 'autumn' ? (
            ...
            <>
              {/* Lighter fog for grassland */}
              <fog attach="fog" args={['#E0F7FA', 30, 100]} />
              <GrasslandScene isRotating={isRotating} />
            </>
          )}
        </Canvas>
      </div>
      ...
      <div className="absolute top-0 left-0 w-full p-6 z-10 pointer-events-none flex flex-col items-start">
        <h1 className="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 via-orange-500 to-red-600 drop-shadow-md">
          {sceneMode === 'autumn' ? 'Autumn Voxel World' : 'Zen Grassland'}
        </h1>
        <p className="text-gray-300 text-sm mt-2 max-w-md drop-shadow-sm">
          {sceneMode === 'autumn' ? 'A vibrant procedural zen garden.' : 'Blue sky, white clouds, and grazing cows.'}
        </p>
      </div>
      ...
      <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 z-20 flex flex-col items-center gap-4 pointer-events-auto w-full px-4">
        ...
        <div className="flex gap-4 flex-wrap justify-center">
          <button
            onClick={() => setSceneMode(prev => prev === 'autumn' ? 'grassland' : 'autumn')}
            className="
          ...
          >
            ...
            <span>Switch to {sceneMode === 'autumn' ? 'Grassland' : 'Autumn'}</span>
          </button>
